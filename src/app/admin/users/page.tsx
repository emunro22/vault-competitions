...
const mockUsers = [
  ...
  { id: '7', name: 'Admin User', email: 'admin@scotcomps.co.uk', role: 'admin' as const, entries: 0, spent: 0, joined: '2025-08-01' },
];
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full sm:w-80 h-11 bg-card border border-border rounded-xl px-4 text-foreground placeholder-muted focus:outline-none focus:border-primary transition-colors text-sm"
        />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left text-xs font-medium text-muted uppercase tracking-wider px-5 py-3 hidden sm:table-cell">Role</th>
                <th className="text-left text-xs font-medium text-muted uppercase tracking-wider px-5 py-3 hidden md:table-cell">Entries</th>
                <th className="text-left text-xs font-medium text-muted uppercase tracking-wider px-5 py-3 hidden lg:table-cell">Total Spent</th>
                <th className="text-left text-xs font-medium text-muted uppercase tracking-wider px-5 py-3 hidden md:table-cell">Joined</th>
                ...
                <tr key={user.id} className="border-b border-border/50 last:border-0 hover:bg-white/[0.02]">
                  ...
                  <td className="px-5 py-4 hidden sm:table-cell">
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-lg ${
                      user.role === 'admin' ? 'bg-primary/10 text-primary-light' : 'bg-background border border-border text-muted'
                    }`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-sm text-muted hidden md:table-cell">{user.entries}</td>
                  <td className="px-5 py-4 text-sm text-foreground hidden lg:table-cell">
                    £{(user.spent / 100).toFixed(2)}
                  </td>
                  <td className="px-5 py-4 text-xs text-muted hidden md:table-cell">
                    {new Date(user.joined).toLocaleDateString('en-GB')}
                  </td>
                  <td className="px-5 py-4 text-right">
                    <button className="text-xs text-primary-light hover:text-primary font-medium transition-colors">
                      View
                    </button>
                  </td>
                </tr>
